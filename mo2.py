...
# Modified in 2019/2020 by B4CKSP4CE members & community <info@0x08.in>
#
    ...
    def checkPublish(self,val):
        ...
        if mqc.initial_connection_made == True:
            ...
            if self.lastval != val:
                self.lastval = val
                try:
                    publish_result = mqc.publish(globaltopic+self.device.name+"/state/"+self.topic,self.lastval,retain=False)
                    if verbosity>=4:
